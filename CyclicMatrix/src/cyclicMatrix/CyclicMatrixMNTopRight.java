...
public class CyclicMatrixMNTopRight {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			try {
				...
				System.out.println("Starting from TOP RIGHT corner!!!");
				...
				if(m >= 1 && m <= 100) {
					...
					if(n>=1 && n<=100) {
						...
						while(i <= m*n) {
							...
							// one cycle
							for(int j = minRow; j <= maxRow; j++) {  //top to bottom
							...
							for(int j = minColumn+1; j <= maxColumn-1; j++) {      // left to right
								matrix[minRow][j] = i++;
							}
